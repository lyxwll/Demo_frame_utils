...
public class GestureImageViewTouchListener implements OnTouchListener {
	...
	public GestureImageViewTouchListener(final GestureImageView image,
			int displayWidth, int displayHeight, ImageDetailViewPager pager) {
		super();
		this.image = image;
		mPager = pager;
		...
		tapDetector = new GestureDetector(image.getContext(),
				new SimpleOnGestureListener() {
					...
					public boolean onDoubleTap(MotionEvent e) {
						if (mPager != null
								&& mPager.getOnViewListener() != null) {
							mPager.getOnViewListener().onDoubleTap();
							if (image.zoom) {
								startZoom(e);
							}
						}
					...
					public boolean onSingleTapConfirmed(MotionEvent e) {
						if (mPager != null
								&& mPager.getOnViewListener() != null) {
							mPager.getOnViewListener().onSingleTapConfirmed();
						}
						if (!inZoom) {
					...
					public void onLongPress(MotionEvent e) {
						if (mPager != null
								&& mPager.getOnViewListener() != null) {
							mPager.getOnViewListener().onLongPress();
						}
						super.onLongPress(e);
					}
					...
					if (event.getPointerCount() > 1) {
						...
						if (!touched) {
							...
							if (handleDrag(event.getX(), event.getY())) {
								...
								if (old.x - event.getX() > 9 && mPager != null) {
									mPager.isLeft(true);
								} else if (old.x - event.getX() < -9
										&& mPager != null) {
									mPager.isRight(true);
								}
							}
