...
public class ScrollLayout extends ViewGroup {
	...
	public void loadControll(int delta, int whichScreen) {
		if (delta > 0 && mCurScreen <= 1) {
			delta = Math.abs(delta) - position;
			mScroller.startScroll(getScrollX(), 0, delta, 0,
					Math.abs(delta) * 2);
			mCurScreen = whichScreen;
			invalidate(); // Redraw the layout
		} else if (delta < 0 && mCurScreen >= getChildCount() - 2) {
	...
	public void snapToScreen(int whichScreen) {
		...
		if (getScrollX() != (whichScreen * getWidth())) {
			...
			if (mCurScreen == whichScreen) {
				if (Math.abs(delta) > position) {
					loadControll(delta, whichScreen);
				} else {
					mScroller.startScroll(getScrollX(), 0, delta, 0,
							Math.abs(delta) * 2);
					mCurScreen = whichScreen;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (velocityX > SNAP_VELOCITY && mCurScreen > 1) {
